...
当前版本不实现 CLI 高频命令、个人偏好和遗忘提醒。团队决策只存放在 Ruyi Serving 服务端 SQLite 数据库中，不写入 OpenClaw 私人记忆文件。
...
flowchart LR
  ...
  B --> C["Ruyi API /v1/chat/completions"]
...
```bash
RUYI_MEMORY_ENGINE_ENABLED=0
```

竞赛演示时开启：

```bash
export RUYI_MEMORY_ENGINE_ENABLED=1
export RUYI_MEMORY_DB_PATH=.run/memory-engine/decision_memory.sqlite3
export RUYI_MEMORY_MAX_CARDS=3
```

## 数据流

1. OpenClaw 把飞书 DM、群聊 `@bot` 或文档片段转成 OpenAI 兼容请求。
2. Ruyi API 复用现有 OpenClaw/飞书 envelope 清洗逻辑，得到真实用户文本。
...
6. 同一租户、项目、会话、主题下的新决策会覆盖旧 active 版本，旧版本标记为 `superseded`。
7. Retriever 只返回 `active` 记忆，并按关键词、主题命中、会话范围、版本新鲜度排序。
8. Prompt Composer 将最多 3 张历史决策卡片注入最终 prompt，并返回 `X-Ruyi-Memory-Hit-Count` 与 `X-Ruyi-Memory-Ids`。
...
在现有 OpenClaw + Feishu + Ruyi Serving 栈中，该能力不改变 compute server 协议，也不新增飞书专用入口。它只增强 API 层的上下文构造，因此可以和现有模型、真实数据集、验收脚本一起运行。
